...
def uxsdcap() -> None:
	...
	capnp_impl_header_file_name = base + "_uxsdcxx_capnp_impl.h"
	...
	with open(capnp_file_name, "w") as capnp_file:
		capnp_file.write(capnp.render_capnp_file(
			schema=schema,
			cmdline=cmdline,
			input_file=input_file))

	with open(capnp_header_file_name, "w") as header_file:
		header_file.write(capnp.render_header_file(
			schema=schema,
			cmdline=cmdline,
	...
	with open(capnp_impl_header_file_name, "w") as impl_header_file:
		impl_header_file.write(capnp.render_impl_header_file(
			schema=schema,
			cmdline=cmdline,
			capnp_file_name=capnp_file_name,
			interface_file_name=interface_file_name,
